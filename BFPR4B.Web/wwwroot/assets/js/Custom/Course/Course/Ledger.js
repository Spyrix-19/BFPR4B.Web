...
var KTDatatablesServerSide = function () {
    ...
    async function resetFilter() {
        ...
        const resetButton = document.querySelector('[data-kt-course-table-filter="reset"]');
        if (resetButton) {
    ...
    async function resetExport() {
        ...
        const modal = document.getElementById('kt_modal_export_course');
        ...
        if (modal) {
            ...
            const resetButton = modal.querySelector('[ data-kt-course-export-action="reset"]');
            if (resetButton) {
                resetButton.addEventListener('click', function (event) {
    ...
    async function openAddCourseModal(detno) {
        // Here, you can use the detno to customize the modal content or perform other actions as needed

        // Change the modal title here
        var modalTitle = document.getElementById("kt_add_course_title");
        modalTitle.innerText = detno === 0 ? "Add Course" : "Update Course"; // Change "New Title" to your desired text

        $('#kt_course_detno').val(detno);
    ...
    // Function to open the "Add Course" modal
    async function openExportCourseModal() {

        $('#kt_modal_export_course').modal('show');

    }

    //// Function to open the "Add Course" modal
    //async function openCourseJournalModal() {

    //    $('#kt_modal_course_journal').modal('show');

    //}

    // Function to open the "Add Rank" modal
    async function openCourseJournalModal(detno) {
        // Here, you can use the detno to customize the modal content or perform other actions as needed

        $('#kt_course_journal_detno').val(detno);
    ...
    async function handleRowDeletion(detno, required) {
        try {
            ...
            if (required) {
                ...
                if (result.value) {
                    const response = await fetch(`/course/remove?detno=${detno}`, {
                        method: 'DELETE',
                        //headers: {
                        //    'Content-Type': 'application/json',
                        //    'Accept': 'application/json'
    ...
    var initDatatable = function () {
        ...
        populateDropdown('COURSE TYPE', 'coursetypeDropdown');

        dt = $("#kt_table_course").DataTable({
            searchDelay: 500,
            processing: true,
            serverSide: false,
            order: [],
            ...
            ajax: {
                ...
                data: function (d) {
                    ...
                    d.parentcode = $("#coursetypeDropdown").val();
            ...
            columns: [
                {
                    ...
                    render: function (data) {
                        console.log("Detno:", data); // Check what value is passed here
                        return `<div class="btn-group">
									...
									<div class="dropdown-menu">
										<div class="dropdown-item px-3">
											<a style="cursor: pointer;" class="menu-link px-3" data-kt-course-table-filter="view_course_journal" data-detno="${data}">
												View Course Journal
											</a>
										</div>
									</div>
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a class="btn btn-sm btn-danger btn-icon btn-icon-md" data-kt-course-table-filter="delete_course" data-toggle="tooltip" data-placement="top" title="Delete" data-detno="${data}" data-required="${reqValue}">
        ...
        $('[data-kt-course-table-filter="add_course"]').on('click', function () {
            const detno = 0;
            // Now you can use the userId to identify the user and open the modal accordingly
            openAddCourseModal(detno);
        });

        // Event handler for opening the "Add User" modal when the button is clicked
        $('#kt_table_course').on('click', '[data-kt-course-table-filter="edit_course"]', function () {
        ...
        // Event handler for clearing search input when x is click
        $('[data-kt-course-table-filter="export_course"]').on('click', function () {
            // Now you can use the userId to identify the user and open the modal accordingly
            openExportCourseModal();
        });

        // Event handler for opening the "Add User" modal when the button is clicked
        $('#kt_table_course').on('click', '[data-kt-course-table-filter="view_course_journal"]', function () {
            const detno = $(this).data('detno');
            // Now you can use the userId to identify the user and open the modal accordingly
            openCourseJournalModal(detno);
        });

        // Event handler for the "x" button in Search
        $('[data-kt-course-table-search="search"]').on('click', resetSearch);

        // Event handler for the "Reset" button in Filter
        $('[data-kt-course-table-filter="reset"]').on('click', resetFilter);

        // Event handler for the "Discard" button in Export
        $('[data-kt-course-export-action="reset"]').on('click', resetExport);

        // Event handler for the "Reset" button in Filter
        $('[data-kt-add-course-modal-action="cancel"]').on('click', resetAddUpdate);

        // Event handler for opening the "Add User" modal when the button is clicked
        $('#kt_table_course').on('click', '[data-kt-course-table-filter="delete_course"]', function () {
            const detno = $(this).data('detno');
