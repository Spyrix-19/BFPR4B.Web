...
var KTAddUpdate = function () {
            ...
            form,
            {
                fields: {
                    'religion_code': {
                        validators: {
                            notEmpty: {
                                message: 'Religion code is required'
                            }
                        }
                    },
                    'religion_name': {
    ...
    var fetchAndUpdateData = async function (detno) {
        try {
            ...
            document.getElementById('kt_religion_detno').value = '';
            document.getElementById('kt_religion_code').value = '';
            document.getElementById('kt_religion_name').value = '';

            const response = await fetch(`/religion/details?detno=${detno}`);
            if (response.ok) {
                const religionData = await response.json();

                RELIGION_DATA = religionData;

                if (religionData !== null) {

                    $('#kt_religion_detno').val(religionData.data.Detno);
                    $('#kt_religion_code').val(religionData.data.Recordcode);
                    $('#kt_religion_name').val(religionData.data.Description);
                }
            } else {
    ...
    async function FetchData() {
        //const dataDetno = document.getElementById('kt_religion_detno').value;
        ...
        const detnoInput = document.getElementById('kt_religion_detno');
    ...
    async function resetAddUpdate() {
            ...
            cancelButton.addEventListener('click', function (event) {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $('#kt_modal_add_religion').modal('hide'); // Hide modal
        ...
        // Check if the button clicked is the "Cancel" button
        const closeButton = document.querySelector('[data-kt-add-religion-modal-action="close"]');
        if (closeButton) {
        ...
        if (status === 'Valid') {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    const addreligionUrl = '/religion/create';
                    ...
                    const requestBody = JSON.stringify({
                        ...
                        recordcode: document.getElementById('kt_religion_code').value,
                        description: document.getElementById('kt_religion_name').value,
                        encodedby: userno,
                    });

                    try {
                        const response = await fetch(addreligionUrl, {
                            method: 'POST',
                            headers: {
                        ...
                        if (response.ok) {
                            ...
                            if (detnojournal !== '0') {
                                const addjournalUrl = '/religion/journal/create';
                                ...
                                if (RELIGION_DATA.data.Recordcode !== null) {
                                    if (RELIGION_DATA.data.Recordcode !== document.getElementById('kt_religion_code').value) {
                                        journalrequestBody.journallist.push({
                                            'description': username + ' : Changed Religion Code from ' + RELIGION_DATA.data.Recordcode + ' to ' + document.getElementById('kt_religion_code').value
                                        });
                                    }
                                }

                                if (RELIGION_DATA.data.Description !== null) {
                                    if (RELIGION_DATA.data.Description !== document.getElementById('kt_religion_name').value) {
                                        journalrequestBody.journallist.push({
                                            'description': username + ' : Changed Religion Name from ' + RELIGION_DATA.data.Description + ' to ' + document.getElementById('kt_religion_name').value
                                        });
                                    }
                                }
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $('#kt_modal_add_religion').modal('hide');
                                    updateDataTable();
                                }
                            });
                        } else {
    ...
    return {
        init: function () {
            form = document.querySelector('#kt_modal_add_religion_form');
            submitButton = document.querySelector('#kt_add_religion_submit');
            modal = document.getElementById('kt_modal_add_religion');
