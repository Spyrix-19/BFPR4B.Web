...
var KTDatatablesServerSide = function () {
    ...
    async function resetFilter() {
        ...
        const resetButton = document.querySelector('[data-kt-rank-table-filter="reset"]');
        if (resetButton) {
    ...
    async function resetExport() {
        ...
        const modal = document.getElementById('kt_modal_export_rank');
        ...
        if (modal) {
            ...
            const resetButton = modal.querySelector('[data-kt-rank-export-action="reset"]');
            if (resetButton) {
                resetButton.addEventListener('click', function (event) {
    ...
    async function openAddRankModal(detno) {
        // Here, you can use the detno to customize the modal content or perform other actions as needed

        // Change the modal title here
        var modalTitle = document.getElementById("kt_add_rank_title");
        modalTitle.innerText = detno === 0 ? "Add Rank" : "Update Rank"; // Change "New Title" to your desired text

        $('#kt_rank_detno').val(detno);

        $('#kt_modal_add_rank').modal('show');


    }

    // Function to open the "Add Course" modal
    async function openExportRankModal() {

        $('#kt_modal_export_rank').modal('show');

    }

    // Function to open the "Add Rank" modal
    async function openRankJournalModal(detno) {
        // Here, you can use the detno to customize the modal content or perform other actions as needed

        $('#kt_rank_journal_detno').val(detno);

        $('#kt_modal_rank_journal').modal('show');
    ...
    async function handleRowDeletion(detno, required) {
        try {
            ...
            if (required) {
                ...
                if (result.value) {
                    const response = await fetch(`/rank/remove?detno=${detno}`, {
                        method: 'DELETE',
                        //headers: {
    ...
    var initDatatable = function () {
        ...
        dt = $("#kt_table_rank").DataTable({
            ...
            columns: [
                        ...
                        return `<div class="btn-group">
									...
									<div class="dropdown-menu">
										<div class="dropdown-item px-3">
											<a style="cursor: pointer;" class="menu-link px-3" data-kt-rank-table-filter="view_rank_journal" data-detno="${data}">
												View Rank Journal
											</a>
										</div>
									</div>
                ...
                },
                {
                    data: 'Detno',
                    render: function (data) {
                        return `<a class="btn btn-sm btn-primary btn-icon btn-icon-md" data-kt-rank-table-filter="edit_rank" data-toggle="tooltip" data-placement="top" title="Change" data-detno="${data}">
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<a class="btn btn-sm btn-danger btn-icon btn-icon-md" data-kt-rank-table-filter="delete_rank" data-toggle="tooltip" data-placement="top" title="Delete" data-detno="${data}" data-required="${reqValue}">
        ...
        // Event handler for clearing search input when x is click
        $('[data-kt-rank-table-filter="export_rank"]').on('click', function () {
            // Now you can use the userId to identify the user and open the modal accordingly
            openExportRankModal();
        });

        // Event handler for opening the "Add User" modal when the button is clicked
        $('#kt_table_rank').on('click', '[data-kt-rank-table-filter="view_rank_journal"]', function () {
            const detno = $(this).data('detno');
            // Now you can use the userId to identify the user and open the modal accordingly
            openRankJournalModal(detno);
        });

        // Event handler for the "x" button in Search
        $('[data-kt-rank-table-search="search"]').on('click', resetSearch);

        // Event handler for the "Reset" button in Filter
        $('[data-kt-rank-table-filter="reset"]').on('click', resetFilter);

        // Event handler for the "Discard" button in Export
        $('[data-kt-rank-export-action="reset"]').on('click', resetExport);

        // Event handler for the "Reset" button in Filter
        $('[data-kt-add-rank-modal-action="cancel"]').on('click', resetAddUpdate);

        // Event handler for opening the "Add User" modal when the button is clicked
        $('#kt_table_rank').on('click', '[data-kt-rank-table-filter="delete_rank"]', function () {
            const detno = $(this).data('detno');
