...
namespace BFPR4B.Web.Controllers.Location
{
	public class RegionController : Controller
	{
		...
		[HttpPost("/region/journal/create")]
		public async Task<IActionResult> CreateRegionJournalAsync([FromBody] CreateRegionJournalDTO parameters)
		{
			try
			{
				if (ModelState.IsValid)
				{
					...
					if (_data.Item1)
					{
						...
						var _response = await _regionService.CreateRegionJournalAsync<APIResponse>(parameters, _data.Item2);
		...
		[HttpGet("/region/journal/ledger")]
		public async Task<IActionResult> GetRegionJournalAsync(string searchkey = "", int regionno = 0, int draw = 1, int start = 1, int length = 20)
		{
			try
			{
				var _data = _accessTokenValidator.ValidateAccessToken(HttpContext);
				...
				if (_data.Item1)
				{
					List<RegionJournalModel> list = new List<RegionJournalModel>();
					...
					var _response = await _regionService.GetRegionJournalAsync<APIResponse>(searchkey, regionno, _data.Item2);

					if (_response != null && _response.IsSuccess)
					{
						list = JsonConvert.DeserializeObject<List<RegionJournalModel>>(Convert.ToString(_response.Result));
